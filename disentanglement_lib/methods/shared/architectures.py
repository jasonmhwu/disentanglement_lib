...
def conv_encoder(input_tensor, num_latent, num_parameters_scale=1, is_training=True, create_logvar=True):
    ...
    e1 = tf.layers.conv2d(
        inputs=input_tensor,
        filters=int(32 * num_parameters_scale),
        kernel_size=4,
        strides=2,
    ...
    e3 = tf.layers.conv2d(
        inputs=e2,
        filters=int(64 * num_parameters_scale),
        kernel_size=2,
        strides=2,
        activation=tf.nn.relu,
        padding="same",
...
def conv_encoder_dropout(
    input_tensor,
    num_latent,
    dropout_rate=gin.REQUIRED,
    num_parameters_scale=1,
    is_training=True
):
    """Convolutional encoder used in beta-VAE paper for the chairs data.
    ...
    Args:
      input_tensor: Input tensor of shape (batch_size, 64, 64, num_channels) to
        build encoder on.
      num_latent: Number of latent variables to output.
      dropout_rate: probability to drop each unit.
      num_parameters_scale: proportion to shrink the model units.
      is_training: Whether or not the graph is built for training (UNUSED).
...
def deconv_decoder(latent_tensor, output_shape, is_training=True, num_parameters_scale=1):
    ...
    """
    del is_training
    d1 = tf.layers.dense(latent_tensor, int(256 * num_parameters_scale), activation=tf.nn.relu)
    d2 = tf.layers.dense(d1, int(1024 * num_parameters_scale), activation=tf.nn.relu)
    d2_reshaped = tf.reshape(d2, shape=[-1, 4, 4, int(64 * num_parameters_scale)])
