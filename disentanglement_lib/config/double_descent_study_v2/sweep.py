...
def get_num_parameters_scales():
    encoder_scales = h.sweep("conv_encoder.num_parameters_scale", h.categorical([i / 32. for i in range(1, 33)]))
    decoder_scales = h.sweep("deconv_decoder.num_parameters_scale", h.categorical([i / 32. for i in range(1, 33)]))
...
def get_default_models():
    ...
    betas = h.sweep("vae.beta", h.discrete([0.125, 1., 8.]))
...
class DoubleDescentStudyV2(study.Study):
    ...
    def get_model_config(self, model_num=0):
        ...
        model_config_file = resources.get_file(
            "config/double_descent_study_v2/model_configs/shared.gin")
    ...
    def get_postprocess_config_files(self):
        ...
        return list(
            resources.get_files_in_folder(
                "config/double_descent_study_v2/postprocess_configs/"))

    def get_eval_config_files(self):
        """Returns evaluation config files."""
        return list(
            resources.get_files_in_folder(
                "config/double_descent_study_v2/metric_configs/"))
