...
def get_default_models():
  ...
  betas = h.sweep("s2_vae.beta", h.discrete([2., 4., 6.]))
  gamma_sups = h.sweep("s2_vae.gamma_sup", h.discrete([1., 2., 4.]))
...
def get_supervised_selection_criterion():
  ...
  return h.sweep(
      ...
      h.categorical([
          "highest_summed_std_all_dims",
          "lowest_summed_std_all_dims",
      ]))
...
class ActiveLearningStudyV3(study.Study):
  """Defines the study for the paper."""
  def __init__(self):
    self.study_name = "active_learning_study_v3"
