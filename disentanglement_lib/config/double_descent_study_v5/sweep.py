...
def get_num_latent():
    return h.sweep("encoder.num_latent", h.categorical([3, 4, 6, 10, 15, 20, 25, 30]))
...
class DoubleDescentStudyV5(study.Study):
    ...
    def get_model_config(self, model_num=0):
        ...
        model_config_file = resources.get_file(
            "config/double_descent_study_v5/model_configs/shared.gin")
    ...
    def get_postprocess_config_files(self):
        ...
        return list(
            resources.get_files_in_folder(
                "config/double_descent_study_v5/postprocess_configs/"))

    def get_eval_config_files(self):
        """Returns evaluation config files."""
        return list(
            resources.get_files_in_folder(
                "config/double_descent_study_v5/metric_configs/"))
