...
def get_datasets():
    ...
    for dataset in datasets:
        ...
        vq_vae_param = h.fixed("vq_vae.factor_sizes", dataset_factor_sizes[dataset])
...
def get_default_models():
    ...
    # VQ-VAE config.
    model_name = h.fixed("model.name", "vq_vae")
    model_fn = h.fixed("model.model", "@vq_vae()")
    betas = h.sweep("vq_vae.beta", h.discrete([1., 4., 16., 64., 128., 256.]))
    gamma_sups = h.sweep("vq_vae.gamma_sup", h.discrete([0.25, 1., 4.]))
...
class EmbeddingsStudyV7(study.Study):
    """Defines the study for the paper."""

    def __init__(self):
        self.study_name = "embeddings_study_v7"
