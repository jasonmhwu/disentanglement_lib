...
def get_datasets():
    ...
    datasets = ["dsprites_full", "smallnorb", "mpi3d_toy","shapes3d", "cars3d"]
...
def get_default_models():
    ...
    # Semi-Supervised Independent VAE
    model_name = h.fixed("model.name", "s2_independent_vae")
    model_fn = h.fixed("model.model", "@s2_independent_vae()")
    betas = h.sweep("s2_independent_vae.beta", h.discrete([1., 4., 16.]))
    gamma_sups = h.sweep("s2_independent_vae.gamma_sup", h.discrete([1.]))
    ind_loss = h.sweep("s2_independent_vae.gamma_ind", h.discrete([0.1, 0.5, 1., 4.]))
    num_stochastic_passes = h.sweep("s2_independent_vae.num_stochastic_passes", h.discrete([1]))
    parameters = h.product([betas, gamma_sups, ind_loss, num_stochastic_passes])
    config_s2_independent_vae = h.zipit([model_name, parameters, model_fn])

    all_models = h.chainit([
        config_s2_independent_vae
    ])
    return all_models
...
def get_supervised_loss():
    ...
    supervised_loss = h.sweep(
        "supervised_loss.loss_fn",
        h.discrete(['xent_and_cov']),
    )
    cov_loss_ratio = h.sweep(
        "supervised_loss.cov_loss_ratio",
        h.discrete([0, 0.5]),
    )
    return h.product([supervised_loss, cov_loss_ratio])
...
class ActiveLearningStudyV20(study.Study):
    """Defines the study for the paper."""

    def __init__(self):
        self.study_name = "active_learning_study_v20"
