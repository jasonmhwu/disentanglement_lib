...
def get_datasets():
    ...
    for dataset in datasets:
        ...
        embedding_dims = h.sweep(
            "mixed.embedding_dimensions",
            h.discrete(embedding_dimensions))
        embedding_losses = h.sweep(
            "mixed.loss_terms",
            h.discrete(loss_terms))
        supervised_info = h.zipit([observed_factors, embedding_dims, embedding_losses])
...
class EmbeddingsStudyV4(study.Study):
    """Defines the study for the paper."""

    def __init__(self):
        self.study_name = "embeddings_study_v4"
