...
def get_num_parameters_scales():
    encoder_scales = h.sweep("conv_encoder.num_parameters_scale", h.discrete([0.0625, 0.125, 0.25, 0.5, 1., 4.]))
    decoder_scales = h.sweep("deconv_decoder.num_parameters_scale", h.discrete([0.0625, 0.125, 0.25, 0.5, 1., 4.]))
...
def get_default_models():
    ...
    betas = h.sweep("vae.beta", h.discrete([0.001, 0.125, 0.25, 0.5, 1., 2., 4., 8., 16.]))
...
class DoubleDescentStudy(study.Study):
    ...
    def get_model_config(self, model_num=0):
        ...
        model_config_file = resources.get_file(
            "config/double_descent_study/model_configs/shared.gin")
    ...
    def get_postprocess_config_files(self):
        ...
        return list(
            resources.get_files_in_folder(
                "config/double_descent_study/postprocess_configs/"))

    def get_eval_config_files(self):
        """Returns evaluation config files."""
        return list(
            resources.get_files_in_folder(
                "config/double_descent_study/metric_configs/"))
